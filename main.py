...
if csv_files:
    ...
    statement = f"select distinct file_name from " \
                f"deproject.product_staging_table " \
...
file_paths =[url[len(prefix):] for url in s3_absolute_file_path]#gives folder and bucket name
logger.info("File path available on s3 under %s bucket and folder name is %s",bucket_name,file_paths)
logger.info(f"File path available ons3 under {bucket_name} bucket and folder name is {file_paths}")
try:
...
if error_files:
    for file_path in error_files:
        if os.path.exists(file_path):
            ...
            logger.info("*****************There is no error file available*************")
#stage table to be updated with status as Active(A)mor inactive(I)

logger.info(f"*************************** Updating staging_table **************************")
insert_statements = []
...
# table = "empty_df_create_table"
# final_df_to_process = spark.read.jdbc(url = config.url,table = table,properties = config.properties)
final_df_to_process.show()

#create new column with concatenated values of extra columns
...
product_table_df = spark.read.format('jdbc'
                       ).option("url", "jdbc:mysql://localhost:3306/deproject"
                                ).option("user", "root"
                                         ).option("password", "Namratha@02"
...
product_staging_table_df = spark.read.format('jdbc'
                       ).option("url", "jdbc:mysql://localhost:3306/deproject"
                                ).option("user", "root"
                                         ).option("password", "Namratha@02"
                                                  ).option('dbtable', 'product_staging_table'
                                                           ).option('driver',
                                                                    'com.mysql.cj.jdbc.Driver'
                                                                    ).load()

logger.info("********************* Loading sales team table into sales_team_table_df ******************")
sales_team_table_df  = spark.read.format('jdbc'
                       ).option("url", "jdbc:mysql://localhost:3306/deproject"
                                ).option("user", "root"
                                         ).option("password", "Namratha@02"
                                                  ).option('dbtable', 'sales_team'
                                                           ).option('driver',
                                                                    'com.mysql.cj.jdbc.Driver'
                                                                    ).load()

logger.info("********************* Loading store table into store_table_df ******************")
store_table_df  = spark.read.format('jdbc'
                       ).option("url", "jdbc:mysql://localhost:3306/deproject"
                                ).option("user", "root"
                                         ).option("password", "Namratha@02"
...
logger.info("*****************************88 write the data into sales team data  mart ********************************")
...
final_customer_data_mart.write \
  .format("jdbc") \
  .option("url", "jdbc:mysql://localhost:3306/deproject") \
  .option("dbtable", "customers_data_mart") \
  .option("user", "root") \
  .option("password", "Namratha@02") \
...
final_sales_team_data_mart_table = final_sales_team_data_mart \
        ...
        .withColumn("incentive", when(col("rnk") == 1,
                                      col("total_sales_month")
                                      *0.01).otherwise(lit(0))) \
        .withColumn("incentive",round(col("incentive"),2)) \
        .withColumn("total_sales", col("total_sales_month")) \
        .select("store_id","sales_person_id","full_name",
                "sales_month","total_sales","incentive")

final_sales_team_data_mart_table.write \
  .format("jdbc") \
  .option("url", "jdbc:mysql://localhost:3306/deproject") \
  .option("dbtable", "sales_team_data_mart") \
  .option("user", "root") \
  .option("password", "Namratha@02") \
  .option("driver", "com.mysql.cj.jdbc.Driver") \
  .mode("overwrite") \
